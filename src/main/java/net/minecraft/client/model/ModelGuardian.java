...
public class ModelGuardian extends ModelBase {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw,
        ...
        guardianBody.rotateAngleY = netHeadYaw / (180F / (float) Math.PI);
        guardianBody.rotateAngleX = headPitch / (180F / (float) Math.PI);
        ...
        for (int i = 0; i < 12; ++i) {
            ...
            guardianSpines[i].rotationPointX = afloat3[i]
                    * (1.0F + MathHelper.cos(ageInTicks * 1.5F + (float) i) * 0.01F - f1);
            guardianSpines[i].rotationPointY = 16.0F
                    + afloat4[i] * (1.0F + MathHelper.cos(ageInTicks * 1.5F + (float) i) * 0.01F - f1);
            guardianSpines[i].rotationPointZ = afloat5[i]
                    * (1.0F + MathHelper.cos(ageInTicks * 1.5F + (float) i) * 0.01F - f1);
        }
