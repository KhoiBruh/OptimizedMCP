...
public class ModelEnderMite extends ModelBase {
    ...
    public ModelEnderMite() {
        ...
        for (int i = 0; i < field_178713_d.length; ++i) {
            ...
            field_178713_d[i].addBox((float) field_178716_a[i][0] * -0.5F, 0.0F,
                    (float) field_178716_a[i][2] * -0.5F, field_178716_a[i][0], field_178716_a[i][1],
                    field_178716_a[i][2]);
            field_178713_d[i].setRotationPoint(0.0F, (float) (24 - field_178716_a[i][1]), f);
    ...
    public void render(Entity entityIn, float p_78088_2_, float p_78088_3_, float p_78088_4_, float p_78088_5_,
        ...
        for (ModelRenderer modelRenderer : field_178713_d) {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw,
        ...
        for (int i = 0; i < field_178713_d.length; ++i) {
            field_178713_d[i].rotateAngleY = MathHelper
                    .cos(ageInTicks * 0.9F + (float) i * 0.15F * (float) Math.PI) * (float) Math.PI * 0.01F
                    * (float) (1 + Math.abs(i - 2));
            field_178713_d[i].rotationPointX = MathHelper
                    .sin(ageInTicks * 0.9F + (float) i * 0.15F * (float) Math.PI) * (float) Math.PI * 0.1F
                    * (float) Math.abs(i - 2);
        }
    }
}
