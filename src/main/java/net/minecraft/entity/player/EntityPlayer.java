...
public abstract class EntityPlayer extends EntityLivingBase {
    ...
    public void onUpdate() {
        ...
        int i = 29999999;
        double d3 = MathHelper.clamp(posX, -2.9999999E7D, 2.9999999E7D);
        double d4 = MathHelper.clamp(posZ, -2.9999999E7D, 2.9999999E7D);
    ...
    public float getToolDigEfficiency(Block p_180471_1_) {
        ...
        if (isPotionActive(Potion.digSlowdown)) {
            float f1 = switch (getActivePotionEffect(Potion.digSlowdown).getAmplifier()) {
                case 0 -> 0.3F;
                case 1 -> 0.09F;
                case 2 -> 0.0027F;
                default -> 8.1E-4F;
            ...
            if (i > 0) {
                switch (ridingEntity) {
                    case EntityMinecart entityMinecart -> {
                        ...
                        } else if (startMinecartRidingCoordinate.distanceSq(MathHelper.floor(posX), MathHelper.floor(posY), MathHelper.floor(posZ)) >= 1000000.0D) {
    ...
    public void clonePlayer(EntityPlayer oldPlayer, boolean respawnFromEnd) {
        if (respawnFromEnd) {
            ...
            lastPortalPos = oldPlayer.lastPortalPos;
            lastPortalVec = oldPlayer.lastPortalVec;
            teleportDirection = oldPlayer.teleportDirection;
