...
public class EntityHorse extends EntityAnimal implements IInvBasic {
    public static final String[] horseTextures = new String[]{"textures/entity/horse/horse_white.png",
    ...
    public void fall(float distance, float damageMultiplier) {
        ...
        if (i > 0) {
            ...
            Block block = worldObj
                    .getBlockState(new BlockPos(posX, posY - 0.2D - (double) prevRotationYaw, posZ))
                    .getBlock();

            if (block.getMaterial() != Material.air && !isSilent()) {
                Block.SoundType block$soundtype = block.stepSound;
                worldObj.playSoundAtEntity(this, block$soundtype.getStepSound(),
    ...
    protected EntityHorse getClosestHorse(Entity entityIn, double distance) {
        ...
        for (Entity entity1 : worldObj.getEntitiesInAABBexcluding(entityIn,
        ...
        if (!blockIn.getMaterial().isLiquid()) {
            ...
            if (riddenByEntity != null && i != 1 && i != 2) {
                ...
                if (gallopTime > 5 && gallopTime % 3 == 0) {
                    playSound("mob.horse.gallop", block$soundtype.getVolume() * 0.15F,
                            block$soundtype.getFrequency());

                    if (i == 0 && rand.nextInt(10) == 0) {
                        playSound("mob.horse.breathe", block$soundtype.getVolume() * 0.6F,
                                block$soundtype.getFrequency());
                    }
                } else if (gallopTime <= 5) {
                    playSound("mob.horse.wood", block$soundtype.getVolume() * 0.15F,
                            block$soundtype.getFrequency());
                }
    ...
    public void openGUI(EntityPlayer playerEntity) {
        if (!worldObj.isRemote && (riddenByEntity == null || riddenByEntity == playerEntity)
                && isTame()) {
            horseChest.setCustomName(getName());
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (itemstack != null && itemstack.getItem() == Items.spawn_egg) {
            ...
            if (itemstack != null) {
                ...
                if (!flag && canCarryChest() && !isChested()
    ...
    public void onUpdate() {
        ...
        if (isRearing()) {
            ...
            rearingAmount += (0.8F * rearingAmount * rearingAmount * rearingAmount
                    - rearingAmount) * 0.6F - 0.05F;
    ...
    private boolean canMate() {
        return riddenByEntity == null && ridingEntity == null && isTame() && isAdultHorse()
                && !isSterile() && getHealth() >= getMaxHealth() && isInLove();
    }
        ...
        if (riddenByEntity != null && riddenByEntity instanceof EntityLivingBase && isHorseSaddled()) {
            ...
            if (jumpPower > 0.0F && !horseJumping && onGround) {
                ...
                if (isPotionActive(Potion.jump)) {
                    motionY += (float) (getActivePotionEffect(Potion.jump).getAmplifier() + 1)
                            * 0.1F;
                }
    ...
    public EntityAgeable createChild(EntityAgeable ageable) {
        ...
        double d2 = getEntityAttribute(horseJumpStrength).getBaseValue()
                + ageable.getEntityAttribute(horseJumpStrength).getBaseValue() + getModifiedJumpStrength();
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata) {
        ...
        if (i != 4 && i != 3) {
            getEntityAttribute(SharedMonsterAttributes.maxHealth)
                    .setBaseValue(getModifiedMaxHealth());

            if (i == 0) {
                getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                        .setBaseValue(getModifiedMovementSpeed());
            } else {
    ...
    protected void spawnHorseParticles(boolean p_110216_1_) {
        ...
        for (int i = 0; i < 7; ++i) {
            ...
            worldObj.spawnParticle(enumparticletypes,
    ...
    public void updateRiderPosition() {
        ...
        if (prevRearingAmount > 0.0F) {
            ...
            riddenByEntity.setPosition(posX + (double) (f2 * f),
                    posY + getMountedYOffset() + riddenByEntity.getYOffset() + (double) f3,
                    posZ - (double) (f2 * f1));
    ...
    private double getModifiedJumpStrength() {
        return 0.4000000059604645D + rand.nextDouble() * 0.2D + rand.nextDouble() * 0.2D
                + rand.nextDouble() * 0.2D;
    }

    private double getModifiedMovementSpeed() {
        return (0.44999998807907104D + rand.nextDouble() * 0.3D + rand.nextDouble() * 0.3D
                + rand.nextDouble() * 0.3D) * 0.25D;
    }
