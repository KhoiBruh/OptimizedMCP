...
public class WorldGenTaiga2 extends WorldGenAbstractTree {
    private static final IBlockState field_181645_a = Blocks.log.getDefaultState().withProperty(BlockOldLog.VARIANT, BlockPlanks.Type.SPRUCE);
    private static final IBlockState field_181646_b = Blocks.leaves.getDefaultState().withProperty(BlockOldLeaf.VARIANT, BlockPlanks.Type.SPRUCE).withProperty(BlockLeaves.CHECK_DECAY, Boolean.FALSE);
                ...
                if ((block1 == Blocks.grass || block1 == Blocks.dirt || block1 == Blocks.farmland) && position.getY() < 256 - i - 1) {
                            ...
                            for (int k2 = position.getZ() - i3; k2 <= position.getZ() + i3; ++k2) {
                                ...
                                if (Math.abs(j2) != i3 || Math.abs(l2) != i3 || i3 <= 0) {
                                    ...
                                    if (!worldIn.getBlockState(blockpos).getBlock().isFullBlock()) {
                                        setBlockAndNotifyAdequately(worldIn, blockpos, field_181646_b);
                                    }
                                }
                            }
                        }
                    ...
                    for (int k4 = 0; k4 < i - i4; ++k4) {
                        ...
                        if (block2.getMaterial() == Material.air || block2.getMaterial() == Material.leaves) {
                            setBlockAndNotifyAdequately(worldIn, position.up(k4), field_181645_a);
                        }
                    }
