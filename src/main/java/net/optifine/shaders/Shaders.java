...
public class Shaders {
            ...
            if (!name.isEmpty() && !name.equals("OFF")) {
                if (name.equals("(internal)")) {
                    ...
                    try {
                        ...
                        return file1.isDirectory() ? new ShaderPackFolder(name, file1)
                                : (file1.isFile() && name.toLowerCase().endsWith(".zip")
    ...
    private static ICustomTexture loadCustomTexture(int textureUnit, String path) {
        if (path == null) {
            ...
            path = path.trim();
            return path.indexOf(58) >= 0 ? loadCustomTextureLocation(textureUnit, path)
                    : (path.indexOf(32) >= 0 ? loadCustomTextureRaw(textureUnit, path)
    ...
    public static int checkGLError(String location) {
        ...
        if (i != 0 && GlErrors.isEnabled(i)) {
            ...
            String s2 = String.format("OpenGL error: %s (%s)%s, at: %s",
                    new Object[] {i, s, s1, location });
            SMCLog.severe(s2);
    ...
    public static boolean isOldHandLight() {
        return !configOldHandLight.isDefault() ? configOldHandLight.isTrue()
                : (shaderPackOldHandLight.isDefault() || shaderPackOldHandLight.isTrue());
    }
    ...
    public static boolean isOldLighting() {
        return !configOldLighting.isDefault() ? configOldLighting.isTrue()
                : (shaderPackOldLighting.isDefault() || shaderPackOldLighting.isTrue());
    }
    ...
    public static void setCameraShadow(float partialTicks) {
        ...
        if (shadowMapIsOrtho) {
            GL11.glOrtho(-shadowMapHalfPlane, shadowMapHalfPlane, -shadowMapHalfPlane, shadowMapHalfPlane,
                    0.05000000074505806D, 256.0D);
        } else {
