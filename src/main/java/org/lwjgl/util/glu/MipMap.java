...
public class MipMap extends Util {
    ...
    public static int gluBuild2DMipmaps(
        ...
        while (!done) {
            ...
            if (null == bufferA) newImage = (bufferA = BufferUtils.createByteBuffer((newW + 4) * newH * bpp));
            else if (null == bufferB) newImage = (bufferB = BufferUtils.createByteBuffer((newW + 4) * newH * bpp));
            ...
            image = newImage;
            if (null != bufferB) bufferB = bufferA;
    ...
    public static int gluScaleImage(
        ...
        switch (typeIn) {
            case GL_UNSIGNED_BYTE -> sizein = 1;
            case GL_FLOAT -> sizein = 4;
            default -> {
        ...
        switch (typeOut) {
            case GL_UNSIGNED_BYTE -> sizeout = 1;
            case GL_FLOAT -> sizeout = 4;
            default -> {
                return GL_INVALID_ENUM;
            }
        }
        ...
        else rowstride = pss.unpackAlignment / sizein * Math.ceilDiv(components * rowlen * sizein, pss.unpackAlignment);
        ...
        if (pss.packAlignment <= sizeout) rowstride = components * rowlen;
        else rowstride = pss.packAlignment / sizeout * Math.ceilDiv(components * rowlen * sizeout, pss.packAlignment);
