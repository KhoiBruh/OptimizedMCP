...
public final class AL {
    ...
    private static void init(String deviceArguments, int contextFrequency, int contextRefresh,
                             boolean contextSynchronized, boolean openDevice) throws LWJGLException {
        try {
            if (openDevice) {
                ...
                if (-1 == contextFrequency) {
                    ...
                    _contextPtr = ALC10.alcCreateContext(var8,
                            createAttributeList(contextFrequency, contextRefresh, var10003, stack));
                    stack.close();
                }
    ...
    private static IntBuffer createAttributeList(int contextFrequency, int contextRefresh,
                                                 int contextSynchronized, MemoryStack stack) {
    ...
    public static void createCapabilities(ALCCapabilities alcCaps) {
        createCapabilities(alcCaps, null);
    }
    ...
    public static void createCapabilities(ALCCapabilities alcCaps,
        ...
        if (NULL == alGetProcAddress) {
            throw new RuntimeException(
                    "A core AL function is missing. Make sure that the OpenAL library has been loaded correctly.");
        }

        FunctionProvider functionProvider = functionName -> {
            long address = invokePP(memAddress(functionName), alGetProcAddress);
            if (NULL == address && Checks.DEBUG_FUNCTIONS) {
        ...
        if (NULL == GetString || NULL == GetError || NULL == IsExtensionPresent) {
            throw new IllegalStateException(
                    "Core OpenAL functions could not be found. Make sure that the OpenAL library has been loaded correctly.");
        }

        String versionString = memASCIISafe(invokeP(AL_VERSION, GetString));
        if (null == versionString || AL_NO_ERROR != invokeI(GetError)) {
        ...
        ALCapabilities caps = new ALCapabilities(functionProvider, supportedExtensions,
                null == bufferFactory ? BufferUtils::createPointerBuffer : bufferFactory);

        if (alcCaps.ALC_EXT_thread_local_context && NULL != alcGetThreadContext()) {
    ...
    private static class ICDStatic implements ICD {
        ...
        public void set(@Nullable ALCapabilities caps) {
            if (null == tempCaps) {
                tempCaps = caps;
            } else if (null != caps && caps != tempCaps
                    && ThreadLocalUtil.areCapabilitiesDifferent(tempCaps.addresses, caps.addresses)) {
